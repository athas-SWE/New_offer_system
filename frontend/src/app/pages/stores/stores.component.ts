...
@Component({
  ...
  template: `
    <div class="page-shell animate-fade-in">
      <h1 class="section-title">Stores</h1>
      <p class="section-sub">Partner merchants listing offers on Offer Lanka.</p>

      <div class="mt-6 max-w-md">
        <input class="input-field" [(ngModel)]="search" (ngModelChange)="load()" placeholder="Search stores or cities…" />
      </div>
      ...
      @if (loading) {
        ...
        <div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          @for (store of stores; track store.id) {
            <a [routerLink]="['/stores', store.id]" class="offer-card-lift surface-panel !p-0 overflow-hidden">
              <div class="flex items-center gap-4 p-4">
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs font-semibold text-gold-600">★ {{ store.rating }} · {{ store.offerCount }} offers</p>
                </div>
              </div>
